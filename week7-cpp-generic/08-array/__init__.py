...
@check50.check()
def exists():
    check50.exists("array.cpp")
    with open("array.cpp") as f:
        sources_buf = f.read()
    return sources_buf

@check50.check(exists)
def compiles():
    check50.c.compile("array.cpp", exe_name="array", cc="g++", ggdb=True, lm=True, std='c++17')

@check50.check(exists)
def validate(sources_buf):
    if "malloc(" in sources_buf or "free(" in sources_buf:
        raise check50.Failure("There should be no calls to malloc/free")
    if "vector<" in sources_buf:
        raise check50.Failure("Should not use vectors")
