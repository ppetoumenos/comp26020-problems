...
def compiles():
    check50.c.compile("inheritance.cpp", exe_name="inheritance", cc="g++", ggdb=True, lm=True, std='c++20')

@check50.check(exists)
def validate(sources_buf):
    if not re.search(r"class\s+Vehicle", sources_buf):
        raise check50.Failure("Could not find class Vehicle")
    if not re.search(r"class\s+Car\s*:\s+public\s+Vehicle", sources_buf):
        raise check50.Failure("Could not find class Car or class Car does not "\
                "inheritate from a parent")
    if not re.search(r"class\s+Motorbike\s*:\s+public\s+Vehicle", sources_buf):
